...
namespace EZPlay.API.Queries
{
    public static class ChoreStatusQueryExecutor
    {
        ...
        public static object Execute(string jsonPayload)
        {
            ...
            var minion = Components.MinionIdentities.Items.FirstOrDefault(m => m.name.Replace("(Clone)", "") == request.DuplicantName);
            if (minion == null)
            {
                return new { error = $"Duplicant '{request.DuplicantName}' not found." };
            }
